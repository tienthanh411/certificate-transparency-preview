...
var (
	...
	etcdHTTPService    = flag.String("etcd_http_service", "trillian-preview-http", "Service name to announce our HTTP endpoint under")
	etcdMetricsService = flag.String("etcd_metrics_service", "trillian-preview-metrics-http", "Service name to announce our HTTP metrics endpoint under")
...
func main() {
	...
	if len(*rpcBackend) > 0 {
		...
		if cfgs, err = preview.LogConfigFromFile(*logConfig); err == nil {
			cfg = preview.ToMultiLogConfig(cfgs, *rpcBackend)
		}
	} else {
		cfg, err = preview.MultiLogConfigFromFile(*logConfig)
	}
	...
	beMap, err := preview.ValidateLogMultiConfig(cfg)
	if err != nil {
		...
		for _, c := range cfg.LogConfigs.Config {
			...
			go func(c *configpb.LogConfig) {
				...
				for t := range ticker.C {
					...
					if err := preview.PingTreeHead(ctx, clientMap[c.LogBackendName], c.LogId, c.Prefix); err != nil {
...
func setupAndRegister(ctx context.Context, client trillian.TrillianLogClient, deadline time.Duration, cfg *configpb.LogConfig, mux *http.ServeMux) error {
	...
	if *quotaIntermediate {
		...
		opts.CertificateQuotaUser = preview.QuotaUserForCert
	}
